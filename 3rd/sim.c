...
// the struct simulating the if_id stage.
struct preg_if_id {
  uint32_t inst;
  uint32_t next_pc;
};
...
// the struct simulating the id_ex stage.
struct preg_id_ex {
...
//the struct simulating the ex_em stage
struct preg_ex_mem {
  bool mem_read; // Whether we should read from memory
  bool mem_write; // Whether we should write from memory
  bool reg_write; // Whether we should write back to a register
...
//the struct simulating the mem_wb stage.
struct preg_mem_wb {
  bool mem_to_reg;
  bool reg_write;
  uint32_t rt;
